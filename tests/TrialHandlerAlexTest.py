...
from inspectFix import inspectTest
inspectTest()
...
stimList = []
for ori in [90,120]: # range(90,180,30):
    for sf in [0.5]: # [0.5, 1.0, 2.0]:
 #     for x in [1.,2.,3.]:
#        for testBooleanVar in [0.0,1.0]:  #it cant aggregate across boolean variables
#           for testBooleanVar2 in [4.0,5.0]:
                stimList.append( 
                    {'sf':sf, 'ori':ori}  # 'x':x, 'testVar':testBooleanVar, 'testVar2': testBooleanVar2} #this is a python 'dictionary'
                    )
...
print '\n'
trials.printAsText(stimOut=['sf'], #write summary data to screen 
...
trials.saveAsExcel(fileName='testData', # ...or an xlsx file (which supports sheets)
                  ...
                  stimOut=['sf','ori'], 
                  dataOut=['RT_mean','RT_std', 'choice_raw'])
trials.saveAsPickle(fileName = 'data/testData')#this saves a copy of the whole object 
