...
#Boot can't deal with a dataframe or list, so boot-related functions will call with returnAsDataframe=FALSE.
  ...
  for (l in lapseRates)	{
	...
	#problem below is that if warning occurs, we don't get the value returned because of error-catching stupidity
  	fitModel<-countWarnings( 
  				binomfit_limsAlex(df$numCorrect,df$numTrials,df$speed,link="logit",
	...
	importantWarns<- importantWarns[ which(attributes(fitModel)$warningMsgs!="non-integer #successes in a binomial glm!") ]
  	...
  	if (method=="brglm.fit") {
  		...
  		deviance<- calcGlmCustomLinkDeviance(fitModel,df$numCorrect,df$numTrials,df$speed,guessing=chanceRate,lapsing=l)
  ...
  cat('method[bestI]=',methods[bestI]," ") #debugON
  cat('linkFxs[bestI]=',linkFxs[bestI]," ") #debugON
...
makeMyPlotCurve4<- function(xmin,xmax,numxs) {#create psychometric curve plotting function over specified domain
  fnToReturn<-function(df) {
    ...
    dh=data.frame(speed=c(.7,1.0,1.4,1.7,2.2),numCorrect=c(46,45,35,26,32),numTrials=c(48,48,48,48,49))
...
makeMyPlotCurve3<- function(xmin,xmax,numxs) {#create psychometric curve plotting function over specified domain
  fnToReturn<-function(df) {
  	#expecting to be passed df with fields:
  	# mean, slope, lapseRate, chanceRate
    df = data.frame(df) #in case it wasn't a dataframe yet
    ...
    dh=data.frame(speed=c(.7,1.0,1.4,1.7,2.2),numCorrect=c(46,45,35,26,32),numTrials=c(48,48,48,48,49))
    dh$lapseRate=df$lapseRate
    exampleModel<- suppressWarnings( binomfit_limsAlex(dh$numCorrect, dh$numTrials, dh$speed, link=linkf, guessing=df$chanceRate, lapsing=df$lapseRate, control=list(maxit=100)) )
    exampleModel=exampleModel$fit
    #modify example fit, use its predictor only plus parameters I've found by fitting
    exampleModel[1]$coefficients[1] = df$mean
    exampleModel[1]$coefficients[2] = df$slope
...
makeMyPredictionPlotCurve<- function(xmin,xmax,numxs,chanceRate,lapseRate) {#create psychometric curve plotting function over specified domain
  fnToReturn<-function(df) {
  	#expecting to be passed df with fields:
  	# mean, slope, lapseRate
	df = data.frame(df) #in case it wasn't a dataframe yet
	#set up example model
	#I don't know why the below didn't work with example01 but it doesn't work
	dh=data.frame(speed=c(.7,1.0,1.4,1.7,2.2),numCorrect=c(36,35,30,26,24),numTrials=c(48,48,48,48,48))
	dh$lapseRate=df$lapseRate
  	exampleModel<- suppressWarnings( binomfit_limsAlex(dh$numCorrect, dh$numTrials, dh$speed, link=linkf, guessing=chanceRate, lapsing=lapseRate, control=list(maxit=100)) )
    exampleModel=exampleModel$fit
	#modify example fit, use its predictor only plus parameters I've found by fitting
  	exampleModel[1]$coefficients[1] = df$mean
  	exampleModel[1]$coefficients[2] = df$slope
	xs = (xmax-xmin) * (0:numxs)/numxs + xmin
	pfit<- suppressWarnings( predict( exampleModel, data.frame( x = xs ), type = "response" ) ) #because of bad previous fit, generates warnings
  	data.frame(speed=xs,correct=pfit)
  }
  return (fnToReturn)
}

makeMyThreshGetNumerically<- function(threshCriterion) {#create function that can use with ddply once have psychometric curves for each condition
...
#		cat(df$numObjects,df$numTargets,df$subject) #debugON
		threshSlop<-threshold_slope (df$correct,df$speed,criterion= threshCriterion)	
  		data.frame(thresh=threshSlop$x_th)
  	}
}
